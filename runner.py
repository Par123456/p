...
class FileManager:
    ...
    def list_files(self):
        try:
            ...
            result = ssh_exec(f"test -d {self.remote_base} && ls -lh {self.remote_base} 2>/dev/null | tail -n +2")
            
            if not result or result.strip() == "":
                return "Directory is empty or not accessible", None
            
            if "cannot" in result.lower() or "no such" in result.lower():
                return None, "Cannot access directory"
    ...
    def get_file_content(self, filename):
        try:
            ...
            result = ssh_exec(f"test -f {path} && cat {path} 2>/dev/null | head -c 100000")
    ...
    def create_file(self, filename, content=""):
        try:
            ...
            ssh_exec(cmd)
            time.sleep(0.5)
            check = ssh_exec(f"test -f {path} && echo 'exists'")
    ...
    def delete_file(self, filename):
        try:
            ...
            time.sleep(0.3)
            check = ssh_exec(f"test -f {path} && echo 'exists' || echo 'deleted'")
            
            if check and "deleted" in check:
                return True, None
            
            return None, "Failed to delete file"
        except Exception as e:
            return None, f"Error deleting file: {str(e)}"
    ...
    def upload_file(self, local_path, remote_filename):
        try:
            ...
            if not local_path or not local_path.strip():
                return None, "Local path cannot be empty"
            ...
            remote_path = f"{self.remote_base}/{remote_filename}"
    ...
    def get_log_content(self, session_id):
        try:
            if not session_id or not session_id.strip():
                return None, "Session ID cannot be empty"
            
            log_path = f"{self.remote_logs}/{session_id}.log"
            result = ssh_exec(f"test -f {log_path} && tail -n 200 {log_path} 2>/dev/null || echo 'No log file'")
            
            if not result or result.strip() == "":
                return None, "No log content found"
    ...
    def cleanup_old_logs(self, days=7):
        try:
            if not self.remote_logs:
                return None, "Remote logs path not configured"
            
            if days < 1:
                return None, "Days must be at least 1"
            
            cmd = f"find {self.remote_logs} -type f -mtime +{days} -delete 2>/dev/null && echo 'cleaned'"
            result = ssh_exec(cmd)
            
            return True, None
        except Exception as e:
            return None, f"Error cleaning logs: {str(e)}"
...
class ExecutionEngine:
    ...
    def get_process_status(self, filename):
        try:
            if not filename or not filename.strip():
                return {"status": "error", "message": "Filename cannot be empty", "count": 0}
            ...
            return {"status": "error", "message": f"Error checking process: {str(e)}", "count": 0}
    ...
    def stop_process(self, identifier):
        try:
            ...
            if not check or len(check.strip()) == 0:
...
class DownloadManager:
    def download_from_git(self, git_url, filename):
        try:
            ...
            if not git_url or not git_url.strip():
                return None, "Git URL cannot be empty"
            
            if not self.remote_base or not self.remote_base.strip():
                return None, "Remote base path not configured"
            
            cmd = f"cd {self.remote_base} && wget -O {filename} '{git_url}' 2>&1"
            result = ssh_exec(cmd, timeout=30)
            
            if "saved" in result.lower() or "100%" in result or "Saving to" in result:
                return filename, None
            
            return None, f"Download may have failed: {result[:100]}"
        except Exception as e:
    ...
    def download_from_url(self, url, filename):
        try:
            ...
            cmd = f"cd {self.remote_base} && curl -L -o {filename} '{url}' 2>&1 && echo 'Downloaded'"
            result = ssh_exec(cmd, timeout=30)
            
            if "Downloaded" in result or "100" in result:
                return filename, None
...
class CommandHandlers:
    ...
    async def cmd_filelist(event, args):
        try:
            ...
            if not result or result.strip() == "":
                return formatter.format_info("FILE LIST", "Directory is empty")
            
            return formatter.format_info("FILE LIST", result)
        except Exception as e:
            return formatter.format_error(f"Error listing files: {str(e)}")
    ...
    async def cmd_addfile(event, args):
        try:
            ...
            return formatter.format_error(f"Error creating file: {str(e)}")
    ...
    async def cmd_adsor(event, args):
        try:
            ...
            if not content or content.strip() == "":
                return formatter.format_error("Reply message is empty")
            ...
            return formatter.format_error(f"Error saving file: {str(e)}")
    ...
    async def cmd_delfile(event, args):
        try:
            ...
            return formatter.format_error(f"Error deleting file: {str(e)}")
    ...
    async def cmd_check(event, args):
        try:
            ...
            if not content or content.strip() == "":
                return formatter.format_info(f"Content: {filename}", "File is empty")
            
            return formatter.format_info(f"Content: {filename}", content)
        except Exception as e:
            return formatter.format_error(f"Error reading file: {str(e)}")
    ...
    async def cmd_run(event, args):
        try:
            ...
            if result.get("status") == "error":
                return formatter.format_error(result.get("message", "Unknown error"))
            
            return formatter.format_success(result.get("message", "Execution complete"), 
                f"Session: {result.get('session_id')}\nLog: {result.get('log_file')}")
        except Exception as e:
            return formatter.format_error(f"Error executing file: {str(e)}")
    ...
    async def cmd_interactive(event, args):
        try:
            ...
            parts = args.split()
            if not parts or not parts[0] or not parts[0].strip():
            ...
            if not prompt:
                return formatter.format_error("Failed to create interactive session")
            
            return formatter.format_info("Interactive Session Started", 
                f"Session ID: {session_id}\n\n{prompt}")
        except Exception as e:
            return formatter.format_error(f"Error starting interactive session: {str(e)}")
    ...
    async def cmd_gitdl(event, args):
        try:
            ...
            url = parts[0].strip()
            filename = parts[1].strip() if len(parts) > 1 else "downloaded_file.py"
            ...
            return formatter.format_error(f"Error downloading from git: {str(e)}")
    ...
    async def cmd_urldl(event, args):
        try:
            ...
            return formatter.format_error(f"Error downloading from URL: {str(e)}")
    ...
    async def cmd_sessions(event, args):
        try:
            ...
            return formatter.format_error(f"Error listing sessions: {str(e)}")
...
Responses Received: {len(session.get('responses', {}))}
    ...
    async def cmd_runlist(event, args):
        try:
            ...
            return formatter.format_error(f"Error listing processes: {str(e)}")
    ...
    async def cmd_kill(event, args):
        try:
            ...
            return formatter.format_error(f"Error killing process: {str(e)}")
    
    @staticmethod
    async def cmd_server_status(event, args):
        try:
            uptime = ssh_exec("uptime") or "Unknown"
            memory = ssh_exec("free -h | grep Mem") or "Unknown"
            disk = ssh_exec("df -h / | tail -1") or "Unknown"
...
{disk}
            ...
            return formatter.format_error(f"Error getting server status: {str(e)}")
...
Active Sessions: {len(session_manager.list_all())}
            ...
            return formatter.format_error(f"Error getting statistics: {str(e)}")
...
🔐 Status: Connected and Ready
            ...
            return formatter.format_error(f"Error in start command: {str(e)}")
...
UTILITIES:
  help                  Show this help
            ...
            return formatter.format_error(f"Error in help command: {str(e)}")
...
async def message_handler(event):
    try:
            ...
            for session_id in list(PENDING_AUTH.keys()):
                ...
                if session and session.get("status") == "awaiting_input":
                    try:
                        ...
                        await event.reply(formatter.format_error("Session processing error", str(e)))
                        return
            return
        ...
        try:
            await event.reply(formatter.format_error("Message processing error", str(e)))
